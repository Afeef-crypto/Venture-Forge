...
import type { UiAgentReport } from "@/types/evaluation";

const search = z.object({ agent: z.string().catch("YC Partner") });
...
function Analysis() {
  ...
  const [list, setList] = useState<UiAgentReport[]>([]);

  useEffect(() => {
    const row = getEvaluation(id);
    setList(row?.agent_results ?? row?.report?.agents ?? []);
  }, [id]);

  const active = list.find((a) => a.name === agent) ?? list[0];

  return (
    <AppShell>
      <Link to="/results/$id" params={{ id }} className="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <div className="mt-7 flex gap-6 overflow-x-auto border-b border-border">
        {list.map((a) => (
          <Link
            ...
            params={{ id }}
            search={{ agent: a.name }}
            className={`shrink-0 border-b-2 pb-3 text-xs ${a.name === active?.name ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}
          >
            {a.name}
          </Link>
        ))}
      ...
      {active && (
        <>
          ...
          <section className="mt-7 border border-border bg-card p-7">
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">{active.analysis}</p>
