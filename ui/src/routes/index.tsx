...
function Index() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background">
      ...
      <nav className="sticky top-0 z-50 w-full border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex h-20 w-full max-w-[1600px] items-center justify-between px-6 lg:px-12">
          ...
          <div className="hidden items-center gap-10 text-sm font-medium md:flex lg:text-base">
            {[["#how", "How It Works"], ["#agents", "Agents"]].map(([href, label], i) => (
              <motion.a
                key={href}
                href={href}
            ...
            <motion.div
              ...
              whileHover={{ y: -2 }}
            >
              <Link to="/docs" className="story-link transition-colors hover:text-primary">
                Docs
              </Link>
            </motion.div>
          </div>
      ...
      {/* === HOW IT WORKS === */}
      <section id="how" className="relative w-full border-t border-border py-20 lg:py-24">
        ...
        <div className="relative mx-auto w-full max-w-[1600px] px-6 lg:px-12">
          <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight sm:text-4xl lg:text-5xl">From idea to <span className="shimmer-title">investor report</span> in three steps.</h2>
            <p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-muted-foreground">A simple loop: you write a few lines, six specialist agents go to work, you get a verdict and a plan.</p>
          </motion.div>

          <div className="relative mt-14 space-y-12 lg:space-y-16">
            ...
            {steps.map(([Icon, num, title, text], i) => {
              ...
              return (
                <motion.article
                  ...
                  className={`relative grid items-center gap-8 lg:grid-cols-2 lg:gap-10 ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}
                >
                  <div className={`${reverse ? "lg:pl-10 lg:text-left" : "lg:pr-10 lg:text-right"}`}>
                    <motion.span initial={{opacity:0,scale:.5}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{delay:.1}} className="font-display text-5xl text-primary/20 lg:text-6xl">{num}</motion.span>
                    <h3 className="mt-1 text-xl font-semibold sm:text-2xl">{title}</h3>
                  ...
                  <div className="relative flex justify-center">
                    <motion.div
                      ...
                      className="relative grid aspect-square w-full max-w-[220px] place-items-center overflow-hidden rounded-xl border border-border bg-card p-6 shadow-lg shadow-primary/10 sm:max-w-[240px]"
                    >
                      ...
                      <motion.span animate={{y:[0,-6,0]}} transition={{repeat:Infinity,duration:3,ease:"easeInOut"}} className="relative grid h-16 w-16 place-items-center rounded-full bg-primary text-primary-foreground shadow-md shadow-primary/30">
      ...
      <section id="docs" className="relative w-full overflow-hidden border-y border-border bg-foreground text-background">
        ...
        <div className="relative mx-auto flex w-full max-w-[1600px] flex-wrap items-center justify-between gap-10 px-6 py-24 lg:px-12">
          ...
          <motion.div whileHover={{scale:1.05,y:-2}} whileTap={{scale:.97}}>
            <Button variant="hero" size="lg" className="btn-shine btn-pulse" asChild><Link to="/dashboard">Open Venture Forge <ArrowRight/></Link></Button>
          </motion.div>
        </div>
      </section>
      ...
      <footer className="w-full px-6 py-12 lg:px-12">
        <div className="mx-auto flex w-full max-w-[1600px] flex-wrap justify-between gap-10">
          ...
          <div className="grid grid-cols-3 gap-16 text-xs">
            <div><b>Product</b><div className="mt-3 space-y-2 text-muted-foreground"><a className="block transition-colors hover:text-primary" href="#agents">Agents</a><a className="block transition-colors hover:text-primary" href="#product">Reports</a></div></div>
            <div><b>Explore</b><div className="mt-3 space-y-2 text-muted-foreground"><a className="block transition-colors hover:text-primary" href="#how">How it works</a><button className="block transition-colors hover:text-primary" onClick={()=>setSampleOpen(true)}>Sample report</button><Link className="block transition-colors hover:text-primary" to="/dashboard">Dashboard</Link></div></div>
            <div><b>Resources</b><div className="mt-3 space-y-2 text-muted-foreground"><Link className="block transition-colors hover:text-primary" to="/docs">Docs</Link><a className="block transition-colors hover:text-primary" href="https://github.com" target="_blank" rel="noreferrer">GitHub</a><a className="block transition-colors hover:text-primary" href="#product">Status</a></div></div>
          </div>
        </div>
      </footer>
